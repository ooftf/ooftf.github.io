...
* onInterceptTouchEvent  
  如果返回 ture 后续事件就不会再调用 onInterceptTouchEvent
* setOnTouchEventListener  -> onTouch  
  如果返回 true 代表消费这个事件，那么就不会再执行 onTouchEvent 方法,如果返回 false 则会调用 onTouchEvent  
  onTouch 方法是在 View 中被调用的，如果是 ViewGroup 那么是在 super.dispatchTouchEvent 中被调用，只有当 子View 没有消费滑动事件，才会调用 super.dispatchTouchEvent 才会间接调用  onTouch
* onTouchEvent  
