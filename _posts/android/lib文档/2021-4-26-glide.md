...
1. 内存缓存
   ...
   2. Lru算法缓存的资源；由 LruResourceCache 实现，上层抽象接口为 MemoryCache， 继承于 LruCache
2. 硬盘缓存 （可配置，默认智能）
   1. 缓存转换后的图片
   2. 缓存原始图片
3. 从网络获取数据  
...
* model
  * 是指我们通过 RequestBuilder.load() 方法传入不同的类型，比如 RUI,"url",ResourceId 等，model 会将他们加载方式的不同，进行分类
* modelLoader
